...
module "github_oidc_policy" {
  ...
  terraform_state_bucket     = module.backend.bucket_name
  lock_table_name            = module.backend.dynamodb_table_name
  cloudfront_distribution_id = module.cloudfront.distribution_id
  hosted_zone_id             = var.hosted_zone_id
  kms_key_arn                = module.s3.kms_key_arn
...
#
# NOTE:
# This infrastructure is created only once. After the backend
# is migrated, Terraform itself will use these resources to
# store its state.
###############################################################
...
module "backend" {
  ...
  dynamodb_table_name = var.terraform_lock_table_name
